...
static void tpd_remove(struct i2c_client *client);
...
static int simu_x = 10;
static int simu_y = 10;
...
static int hello_proc_open(struct inode *inode, struct  file *file) {
  ...
  tpd_down(simu_x,simu_y, 1, 1);
  tpd_down(simu_x + 10,simu_y, 2, 1);
  tpd_down(simu_x + 20,simu_y, 3, 1);
  tpd_down(simu_x + 30,simu_y, 4, 1);
  input_sync(tpd->dev);
  msleep(100);
  //tpd_up(simu_x,simu_y, 1);
  simu_x = (simu_x + 50) %500;
  simu_y = (simu_y + 50) %500;;
...
{
	UNUSED(mtk_ctp_attr_group);
	UNUSED(touchscreen_dev_dir);
	UNUSED(virtual_dir);
	UNUSED(touchscreen_dir);
#if ACER_GESTURE_WAKEUP
	proc_init();
#endif
