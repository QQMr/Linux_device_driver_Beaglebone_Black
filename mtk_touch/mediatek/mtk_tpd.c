...
{
	int res = 0;
	TPD_DMESG("MediaTek touch panel init\n");
	tpd_init_workqueue = create_singlethread_workqueue("mtk-tpd");
	//INIT_WORK(&tpd_init_work, tpd_init_work_callback);
	INIT_DELAYED_WORK(&ms_queue_work, tpd_init_work_callback);
	//res = queue_work(tpd_init_workqueue, &tpd_init_work);
	res = queue_delayed_work(tpd_init_workqueue, &ms_queue_work, msecs_to_jiffies(10 * 1000));
	if (!res)
