			       ...
			       const struct i2c_device_id *dev_id)
{
	...
	dev_dbg(&client->dev,"synaptics_i2c_touch_create:\n");
	touch = synaptics_i2c_touch_create(client);
	if (!touch)
		return -ENOMEM;

	dev_dbg(&touch->client->dev,"synaptics_i2c_reset_config:\n");
	ret = synaptics_i2c_reset_config(client);
	if (ret)
		goto err_mem_free;
	...
	dev_dbg(&touch->client->dev,"input_allocate_device:\n");
	...
	dev_dbg(&touch->client->dev,"synaptics_i2c_set_input_params:\n");
	...
	dev_dbg(&touch->client->dev,"polling_req:\n");
	if (polling_req)
		dev_dbg(&touch->client->dev,
			 "Using polling at rate: %d times/sec\n", scan_rate);

	/* Register the device in input subsystem */
	dev_dbg(&touch->client->dev,"input_register_device:\n");
